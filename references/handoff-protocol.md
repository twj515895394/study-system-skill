...
`CURRENT.md`、`PROGRESS.md`、`COURSE-LIST.md`、最新 handoff 必须使用统一状态模型，状态取值见 `references/status-model.md`。

关键约束：

1. 如果当前状态是 `待理解检查`、`待补课` 或 `补课中`，不要直接进入下一课。
2. 只有状态为 `已完成`，才允许写入 `learning-records/` 和 `GLOSSARY.md`。
3. 如果最新 handoff 与 `CURRENT.md` / `PROGRESS.md` 状态冲突，优先停下来同步状态，而不是继续授课。
